...
public class LazyStateTrackingProvider {
	...
	public LazyStateTrackingProvider(WhyDidYouRenderConfig config) {
		...
		_stateComparer = new Lazy<StateComparer>(
			() => new StateComparer(),
			LazyThreadSafetyMode.ExecutionAndPublication);
	...
	public async Task PreWarmCacheAsync(IEnumerable<Type> componentTypes) {
		...
		var tasks = componentTypes
			.Where(t => typeof(ComponentBase).IsAssignableFrom(t))
			.Select(t => analyzer.AnalyzeComponentTypeAsync(t));
	...
	public StateTrackingDiagnostics GetDiagnostics() {
		return new StateTrackingDiagnostics {
			...
			CacheInfo = _isInitialized && _stateFieldAnalyzer.IsValueCreated
				? _stateFieldAnalyzer.Value.GetCacheInfo()
				: null,
			PerformanceSummary = _isInitialized && _performanceMonitor.IsValueCreated
				? _performanceMonitor.Value.GetPerformanceSummary()
				: null
		};
	}
