...
public record AttributeSummary {
    ...
    /// </summary>
    public bool HasExplicitConfiguration => StateTrackingOptions != null ||
                                           ExplicitlyTrackedMembers.Count > 0 ||
                                           ExplicitlyIgnoredMembers.Count > 0;

    /// <summary>
    /// Gets a brief description of the tracking configuration.
    /// </summary>
    public string ConfigurationSummary => HasIgnoreStateTracking
        ? "State tracking disabled"
        : $"{TotalTrackedMembers} tracked members ({TrackingCoverage:F1}% coverage)";
    ...
    public string GetFormattedSummary() {
        var summary = $"Attribute Summary for {ComponentType.Name}:\n" +
