...
public record OperationTiming {
    ...
    /// </summary>
    public string OutcomeDescription => Success
        ? $"Success ({DurationMs:F2}ms)"
        : $"Failed ({DurationMs:F2}ms): {ExceptionType}";
    ...
    public string GetFormattedSummary() {
        ...
        var result = $"[{status}] {OperationName}\n" +
    ...
    public static OperationTiming CreateSuccess(string operationName, DateTime startTime, TimeSpan duration) => new() {
    ...
    public static OperationTiming CreateFailure(string operationName, DateTime startTime, TimeSpan duration, Exception exception) => new() {
        OperationName = operationName,
        StartTime = startTime,
        Duration = duration,
        Success = false,
