...
public record PerformanceSummary
{
    ...
    /// </summary>
    public double OverallSuccessRate => TotalOperations > 0 
    ...
    /// </summary>
    public double OverallFailureRate => TotalOperations > 0 
        ? (double)TotalFailedOperations / TotalOperations 
        : 0.0;
    ...
    /// </summary>
    public double OperationsPerSecond => MonitoringDuration.TotalSeconds > 0 
        ? TotalOperations / MonitoringDuration.TotalSeconds 
        : 0.0;
    ...
    /// </summary>
    public bool HasPerformanceIssues => OverallFailureRate > 0.05 || 
    ...
    public IEnumerable<OperationMetrics> ProblematicOperations => 
        OperationMetrics.Values.Where(m => m.HasPerformanceIssues)
    ...
    {
        var summary = $"Performance Summary (Monitoring Duration: {MonitoringDuration}):\n" +
        ...
        if (SlowestOperation != null)
            summary += $"  Slowest Operation: {SlowestOperation.OperationName} ({SlowestOperation.AverageTime.TotalMilliseconds:F2}ms avg)\n";

        if (FastestOperation != null)
            summary += $"  Fastest Operation: {FastestOperation.OperationName} ({FastestOperation.AverageTime.TotalMilliseconds:F2}ms avg)\n";
