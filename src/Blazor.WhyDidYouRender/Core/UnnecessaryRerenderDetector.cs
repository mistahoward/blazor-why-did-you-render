...
public class UnnecessaryRerenderDetector
{
		...
		try
		{
			...
			var fields = componentType
				...
				.Where(f =>
					...
					&& // skip auto-property backing fields
					f.FieldType != typeof(RenderTrackerService)
				) // skip our tracker
				.ToList();
	...
	private static bool AreStatesEquivalent(object? previous, object? current)
	{
		...
		{
			if (prevDict.Count != currDict.Count)
				return false;

			foreach (var kvp in prevDict)
			{
				if (!currDict.TryGetValue(kvp.Key, out var currValue))
					return false;
				if (!Equals(kvp.Value, currValue))
					return false;
			}
	...
	public StateSnapshotManager? GetStateSnapshotManager() => _lazyStateTrackingProvider?.SnapshotManager;
	...
	public UnnecessaryRerenderStatistics GetStatistics() =>
		new() { TrackedComponents = _componentStates.Count, IsEnabled = _config.DetectUnnecessaryRerenders };
	...
	public StateTrackingDiagnostics? GetStateTrackingDiagnostics() => _lazyStateTrackingProvider?.GetDiagnostics();

	/// <summary>
	/// Performs maintenance on state tracking components.
	/// </summary>
	public void PerformStateTrackingMaintenance() => _lazyStateTrackingProvider?.PerformMaintenance();

	/// <summary>
	/// Resets all state tracking components to their initial state.
	/// </summary>
	public void ResetStateTracking() => _lazyStateTrackingProvider?.Reset();
}
