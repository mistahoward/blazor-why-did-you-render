...
public static class ServiceCollectionExtensions
{
	...
	public static IServiceCollection AddWhyDidYouRender(this IServiceCollection services, IConfiguration? configuration = null)
	{
		var config = new WhyDidYouRenderConfig();
		configuration?.Bind(config);

		return AddWhyDidYouRender(
			services,
			c =>
			{
	...
	/// <returns>The service collection for chaining.</returns>
	public static IServiceCollection AddWhyDidYouRender(this IServiceCollection services, Action<WhyDidYouRenderConfig> configureOptions)
	{
		var config = new WhyDidYouRenderConfig();
		...
		// Register unified logger interface
		services.AddSingleton<Logging.IWhyDidYouRenderLogger>(provider =>
		{
			...
			// Server/SSR paths with safe fallback
			var melLogger = provider.GetService<Microsoft.Extensions.Logging.ILogger<Logging.ServerWhyDidYouRenderLogger>>();
			Logging.IWhyDidYouRenderLogger baseLogger =
				melLogger != null
					? new Logging.ServerWhyDidYouRenderLogger(config, melLogger)
					: new Logging.ConsoleWhyDidYouRenderLogger(config);

			if (config.EnableOpenTelemetry)
			{
				var otelLogger = new Logging.AspireWhyDidYouRenderLogger(config);
				return new Logging.CompositeWhyDidYouRenderLogger(config, baseLogger, otelLogger);
			}
	...
	public static async Task InitializeWhyDidYouRenderAsync(this IServiceProvider serviceProvider, IJSRuntime jsRuntime)
	{
	...
	private static void RegisterEnvironmentSpecificServices(IServiceCollection services, WhyDidYouRenderConfig config)
	{
			...
			if (config.ForceHostingModel.HasValue)
			{
				return config.ForceHostingModel.Value switch
				{
					...
					_ => new ServerSessionContextService(
						...
						config,
						provider.GetService<Logging.IWhyDidYouRenderLogger>()
					),
				};
			}

			return detector.IsClientSide
				? new WasmSessionContextService()
				: new ServerSessionContextService(
					provider.GetService<Microsoft.AspNetCore.Http.IHttpContextAccessor>()!,
					config,
					provider.GetService<Logging.IWhyDidYouRenderLogger>()
				);
		...
		services.AddSingleton<IErrorTracker>(provider =>
		{
			...
			if (config.ForceHostingModel.HasValue)
				return config.ForceHostingModel.Value switch
				{
					BlazorHostingModel.WebAssembly => new WasmErrorTracker(provider.GetRequiredService<IJSRuntime>(), config),
	...
	public static void InitializeSSRServices(this IServiceProvider serviceProvider)
	{
		...
		{
			tracker.SetErrorTracker(newErrorTracker);
			LogInfo(serviceProvider, $"Error tracker initialized: {newErrorTracker.ErrorTrackingDescription}");
		}
		else
			LogError(serviceProvider, "Error tracker service not found!");
		...
		if (sessionContextService != null)
		{
			...
			LogInfo(serviceProvider, "Session context service initialized.");
		}
		else
		...
		if (unified != null)
			RenderTrackerService.SetUnifiedLogger(unified);
		else
		{
			LogError(serviceProvider, "Unified logger service not found!");
		}
	...
	private static void LogInfo(IServiceProvider sp, string message, Dictionary<string, object?>? data = null)
	{
		...
		if (logger != null)
			...
			Console.WriteLine($"[WhyDidYouRender] {message}");
	}
	...
	private static void LogWarning(IServiceProvider sp, string message, Dictionary<string, object?>? data = null)
	{
		...
		if (logger != null)
			...
			Console.WriteLine($"[WhyDidYouRender] WARNING: {message}");
	}
	...
	private static void LogError(IServiceProvider sp, string message, Exception? exception = null, Dictionary<string, object?>? data = null)
	{
		var logger = sp.GetService<Logging.IWhyDidYouRenderLogger>();
		if (logger != null)
			logger.LogError(message, exception, data);
		else
			Console.WriteLine(
				exception == null ? $"[WhyDidYouRender] ERROR: {message}" : $"[WhyDidYouRender] ERROR: {message} | {exception.Message}"
			);
	}
