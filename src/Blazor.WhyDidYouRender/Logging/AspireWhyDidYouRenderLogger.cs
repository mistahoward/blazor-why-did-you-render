...
public class AspireWhyDidYouRenderLogger : WhyDidYouRenderLoggerBase
{
	...
	public AspireWhyDidYouRenderLogger(WhyDidYouRenderConfig config)
		: base(config)
	{
		...
		_unnecessaryCounter = _meter.CreateCounter<long>(
			"wdyrl.rerenders.unnecessary",
			unit: "count",
			description: "Unnecessary rerenders"
		);
		_renderDurationMs = _meter.CreateHistogram<double>(
			"wdyrl.render.duration.ms",
			unit: "ms",
	...
	public override void LogDebug(string message, Dictionary<string, object?>? data = null) => WriteActivity("Debug", message, data);

	/// <summary>Writes an informational message as an Aspire/OpenTelemetry activity.</summary>
	public override void LogInfo(string message, Dictionary<string, object?>? data = null) => WriteActivity("Info", message, data);

	/// <summary>Writes a warning message as an Aspire/OpenTelemetry activity.</summary>
	public override void LogWarning(string message, Dictionary<string, object?>? data = null) => WriteActivity("Warning", message, data);
	...
	public override void LogRenderEvent(Records.RenderEvent renderEvent)
	{
		...
		if (
			_config.ComponentWhitelist != null
			&& _config.ComponentWhitelist.Count > 0
			&& !_config.ComponentWhitelist.Contains(renderEvent.ComponentName)
		)
			return;
		...
		if (activity != null)
		{
			...
			if (renderEvent.FirstRender.HasValue)
				activity.SetTag("wdyrl.first_render", renderEvent.FirstRender.Value);
			...
			if (!string.IsNullOrEmpty(renderEvent.UnnecessaryRerenderReason))
				activity.SetTag("wdyrl.reason", renderEvent.UnnecessaryRerenderReason);
		...
		if (_config.EnableOtelMetrics)
		{
			...
			{
				if (!string.IsNullOrEmpty(renderEvent.UnnecessaryRerenderReason))
					tags.Add("reason", renderEvent.UnnecessaryRerenderReason);
