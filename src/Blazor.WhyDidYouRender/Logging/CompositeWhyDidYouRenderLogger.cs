...
public class CompositeWhyDidYouRenderLogger : WhyDidYouRenderLoggerBase {
    ...
    public override void LogRenderEvent(Records.RenderEvent renderEvent) {
        ...
        if (activity == null) {
            foreach (var l in _inner) l.LogRenderEvent(renderEvent);
            return;
        }
        try {
    ...
    public override void LogParameterChanges(string componentName, Dictionary<string, object?> changes) {
        foreach (var l in _inner) l.LogParameterChanges(componentName, changes);
    }
    ...
    public override void LogPerformance(string componentName, string method, double durationMs, Dictionary<string, object?>? metrics = null) {
        foreach (var l in _inner) l.LogPerformance(componentName, method, durationMs, metrics);
    }

    public override void LogStateChange(string componentName, string fieldName, object? previousValue, object? currentValue, string changeType) {
        foreach (var l in _inner) l.LogStateChange(componentName, fieldName, previousValue, currentValue, changeType);
    }

    public override void LogUnnecessaryRerender(string componentName, string reason, Dictionary<string, object?>? context = null) {
        foreach (var l in _inner) l.LogUnnecessaryRerender(componentName, reason, context);
    }

    public override void LogFrequentRerender(string componentName, int renderCount, TimeSpan timeSpan, Dictionary<string, object?>? context = null) {
        foreach (var l in _inner) l.LogFrequentRerender(componentName, renderCount, timeSpan, context);
    }

    public override void LogInitialization(string componentName, Dictionary<string, object?>? config = null) {
        foreach (var l in _inner) l.LogInitialization(componentName, config);
    }

    public override void LogDisposal(string componentName) {
        foreach (var l in _inner) l.LogDisposal(componentName);
    }

    public override void LogException(string componentName, Exception exception, string operation, Dictionary<string, object?>? context = null) {
        foreach (var l in _inner) l.LogException(componentName, exception, operation, context);
    }
}
