...
public record CacheInfo
{
    ...
    /// </summary>
    public TimeSpan TimeSinceLastAccess => MostRecentlyAccessed.HasValue 
        ? DateTime.UtcNow - MostRecentlyAccessed.Value 
        : TimeSpan.MaxValue;
    ...
    /// </summary>
    public bool HasStaleEntries => LeastRecentlyAccessed.HasValue && 
                                  DateTime.UtcNow - LeastRecentlyAccessed.Value > TimeSpan.FromHours(1);

    /// <summary>
    /// Gets the cache efficiency rating based on hit rate and memory usage.
    /// </summary>
    public string EfficiencyRating => Statistics.HitRate switch
    {
    ...
    public static CacheInfo Create(
        ...
        CacheStatistics? statistics = null) => new()
    {
    ...
    {
        var summary = $"Cache Information (Efficiency: {EfficiencyRating}):\n" +
