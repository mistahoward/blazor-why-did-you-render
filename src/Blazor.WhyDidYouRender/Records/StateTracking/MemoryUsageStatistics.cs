...
public record MemoryUsageStatistics
{
	...
	/// </summary>
	public double CleanupEfficiency => CleanupOperations > 0 ? (double)ComponentsCleanedUp / CleanupOperations * 100 : 0;

	/// <summary>
	/// Gets the average memory freed per garbage collection (in bytes).
	/// </summary>
	public double AverageMemoryFreedPerGC => GarbageCollections > 0 ? (double)TotalMemoryFreed / GarbageCollections : 0;
