...
public record StateComparisonResult
{
	...
	/// </summary>
	public double ChangePercentage => TotalFieldsCompared > 0 ? (double)ChangedFieldCount / TotalFieldsCompared * 100 : 0.0;

	/// <summary>
	/// Gets field comparisons that resulted in changes.
	/// </summary>
	public IEnumerable<FieldComparisonResult> ChangedFieldComparisons => FieldComparisons.Values.Where(fc => fc.HasChanged);

	/// <summary>
	/// Gets field comparisons that resulted in no changes.
	/// </summary>
	public IEnumerable<FieldComparisonResult> UnchangedFieldComparisons => FieldComparisons.Values.Where(fc => !fc.HasChanged);

	/// <summary>
	/// Gets field comparisons that represent meaningful changes (not just reference changes).
	/// </summary>
	public IEnumerable<FieldComparisonResult> MeaningfulChanges => FieldComparisons.Values.Where(fc => fc.IsMeaningfulChange);
	...
	public static StateComparisonResult NoChanges(IReadOnlyDictionary<string, FieldComparisonResult> fieldComparisons) =>
		new()
		{
			HasChanges = false,
	...
	{
		var changedFields = fieldComparisons.Values.Where(fc => fc.HasChanged).Select(fc => fc.FieldName).ToList();
	...
	public static StateComparisonResult Empty() =>
		new()
		{
			HasChanges = false,
			ChangedFields = Array.Empty<string>(),
