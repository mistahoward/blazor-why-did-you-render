...
public class StressTestResult {
    ...
    /// </summary>
    public TimeSpan AverageTimePerOperation => OperationsCompleted > 0
        ? TimeSpan.FromTicks(Duration.Ticks / OperationsCompleted)
        : TimeSpan.Zero;

    /// <summary>
    /// Gets whether the test performance is considered good based on common thresholds.
    /// </summary>
    public bool IsGoodPerformance => Success &&
                                    OperationsPerSecond > 100 &&
                                    AverageTimePerOperation < TimeSpan.FromMilliseconds(10);
    ...
    /// </summary>
    public string PerformanceRating => OperationsPerSecond switch {
        > 1000 => "Excellent",
        > 500 => "Good",
        > 100 => "Fair",
        > 10 => "Poor",
    ...
    public static StressTestResult CreateSuccess(string testName, TimeSpan duration, int operationsCompleted, long memoryUsed = 0) => new() {
        TestName = testName,
        Success = true,
        Duration = duration,
    ...
    public static StressTestResult CreateFailure(string testName, TimeSpan duration, string errorMessage, Exception? exception = null, int operationsCompleted = 0) => new() {
        TestName = testName,
        Success = false,
        Duration = duration,
        OperationsCompleted = operationsCompleted,
    ...
    public string GetFormattedSummary() {
        var result = $"Stress Test Result: {TestName}\n" +
                    $"  Status: {Status}\n" +
